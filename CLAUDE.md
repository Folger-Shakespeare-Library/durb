...
- **Batching:** When any `--with` extras are requested, `GetConstituentFull` uses the Tessitura `/api/Batch` endpoint to fetch detail + all extras in a single HTTP call. Fixed request IDs: 1=detail, 2=affiliations (individual), 3=affiliations (group), 4=notes. When adding new `--with` options, assign the next available ID and add a case in the switch.
- **Concurrent fetching:** Multiple constituent IDs are fetched with goroutines in `GetConstituentsBatch`.
